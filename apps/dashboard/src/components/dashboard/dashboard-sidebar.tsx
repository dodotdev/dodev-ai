...
const SUB_ITEMS = [
  ...
  },
  {
    ...
    label: "Memories",
    icon: Brain,
    to: "/dashboard/spaces/$id/memories",
    suffix: "/memories",
  },
  {
    key: "activity",
    label: "Activity",
    icon: Activity,
    to: "/dashboard/spaces/$id/activity",
    suffix: "/activity",
  },
  {
    key: "settings",
    label: "Settings",
    icon: Settings,
    to: "/dashboard/spaces/$id/settings",
    suffix: "/settings",
  },
] as const
...
export function DashboardSidebar({ onNavigate }: DashboardSidebarProps) {
  ...
  const { user, apiKeyHash, signOutUrl } = useAuth()
      ...
      <NewSpaceDialog
        ...
        onCreate={async (data) => {
          ...
          if (result) {
            ...
            navigate({ to: "/dashboard/spaces/$id", params: { id } })
            onNavigate?.()
          }
    ...
    <div>
      ...
      {isExpanded && (
        <div className="ml-[22px] space-y-1 border-l border-border py-1 pl-3">
          {SUB_ITEMS.map((sub) => {
            const Icon = sub.icon
            const href = `${base}${sub.suffix}`
            const active =
              sub.suffix === "" ? pathname === base : pathname === href || pathname.startsWith(href)
